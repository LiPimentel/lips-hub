...
15. **(business-analyst, 2026-09-08) El estado vacío de Inicio ("Agrega un producto para empezar", RF 13) no se pudo llevar a pantalla por la interfaz real, porque no hay forma de bajar a 0 productos activos.** El único camino real para quitar un producto es "Archivar" en Productos y metas (RF 49), y ese botón está explícitamente condicionado a que haya más de un producto — con un solo producto activo (el caso de partida de cualquier cuenta nueva) no aparece la opción de archivarlo. Esto significa que, tal como está construida hoy la app, ese mensaje de estado vacío es código que ningún usuario real puede llegar a ver a través de la interfaz normal. **Pendiente de decisión del tech lead:** o bien es un estado defensivo intencional que puede quedar sin uso (por ejemplo, por si en el futuro se agrega otra forma de quitar productos, o por una inconsistencia de datos), o bien conviene revisar si el mensaje/código muerto debe eliminarse o si en cambio debería habilitarse alguna vía real para llegar a "0 productos" de forma controlada.

16. **(business-analyst, 2026-09-08) Las insignias "Semana perfecta" y "Sin congelar" (visibles en Perfil, RF 44) nunca se pueden desbloquear, sin importar lo que haga la persona usuaria.** Confirmado por lectura de `refreshBadges()`: de las 14 insignias listadas en pantalla, 12 tienen una condición real evaluada en código y estas 2 no tienen ninguna — quedan permanentemente en estado "Bloqueada" aunque la persona registre consumo todos los días de una semana calendario o nunca use ningún día libre. Es un riesgo de confianza en el producto, no solo un detalle cosmético: alguien que cumpla exactamente lo que el nombre de la insignia promete (una semana perfecta de registros, o terminar el reto sin usar días libres) puede razonablemente pensar que la app está fallando o perdiendo su progreso al ver que la insignia sigue bloqueada. **Pendiente de decisión del tech lead:** implementar la regla de desbloqueo real para ambas, o si no se van a implementar pronto, ocultarlas/marcarlas como "próximamente" en vez de mostrarlas como una meta alcanzable que nunca se cumple.

17. **(business-analyst, 2026-09-08) No existe en toda la aplicación ningún control para cambiar cuál producto está activo, una vez que hay más de uno — ni siquiera en la pantalla que lista los productos.** Confirmado por lectura de código y por navegación real de Productos y metas (RF 50): las pestañas de esa pantalla ("Urth Superfood Brew", el producto propio agregado, etc.) sí permiten *ver* la configuración de cada producto, pero el clic no toca `state.activeProductId` — y ese es el único valor que determina qué producto se usa en Inicio, Recetas, Registro de consumo, Nutrición, Análisis y Reto. En la práctica, si una persona agrega un segundo producto (camino que el propio onboarding invita a tomar, RF 2), no tiene ninguna manera de volver a operar sobre el primero desde la interfaz una vez que el segundo pasó a ser el activo (por ejemplo, tras archivar y reasignar automáticamente, RF 49) — ni de elegir manualmente cuál usar cada día si tiene dos productos en uso alternado. **Pendiente de decisión del tech lead:** esto no es un caso límite raro sino un hueco funcional central si la app pretende soportar más de un producto en paralelo — se necesita definir dónde y cómo se agrega ese control (por ejemplo, un clic en la pestaña de Productos y metas que si active el producto, o un selector aparte en el sidebar).

18. **(release-manager, 2026-09-08, riesgo de infraestructura, no específico de esta app pero relevante para su primer merge) La corrida programada del 2026-09-07 del keepalive de Supabase (`.github/workflows/supabase-keepalive.yml`, la que evita que el proyecto gratuito se auto-pause por inactividad) falló con "Process completed with exit code 6"** — compatible con que el runner de GitHub Actions no pudo ni siquiera resolver el nombre de dominio de Supabase (curl exit 6 = "Couldn't resolve host"), no un simple rechazo HTTP. Las 9 corridas anteriores (2026-08-06 a 2026-09-03) están en verde. Esta sesión no pudo confirmar la causa real ni el estado actual del proyecto: el proxy de este entorno bloquea tanto `*.supabase.co` como `*.workers.dev` (política de organización, `connect_rejected`), así que no hay forma de verificar de forma directa desde aquí si el proyecto sigue activo, está pausado, o fue un bache transitorio de red del lado de GitHub. **No es un caso borde de la app Nutri Log en sí** — cualquiera de las 7 apps del hub quedaría sin poder autenticar si el proyecto se pausó — pero se registra aquí porque esta revisión de merge de Nutri Log fue la que lo encontró. Ver detalle, urgencia y recomendación en `docs/infra-watch.md` y `docs/team-memory.md` ("⚠️ Requiere atención").
