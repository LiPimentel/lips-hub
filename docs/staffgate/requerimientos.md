...
12. **(security-reviewer, 2026-07-23) Ventana de carrera estrecha en la que `save()` todavía puede escribir en la clave local sin aislar por usuario, pese a la corrección de mezcla de cuentas.** El botón "+ Nuevo candidato" (línea 792, `onclick="openNewModal()"`) es HTML estático, visible e interactivo desde que la página pinta — no depende de que `load()` haya terminado. `cachedUserId` (línea 975) solo se fija dentro de `load()` (línea 1028), que es asíncrono. Si `createCandidate()` (línea 1874) se dispara antes de que esa asignación ocurra (por ejemplo, si `getSupabaseSession()` tarda por una renovación de token), `save()` (línea 989-991) escribe con `scopedKey('staffgate_cands_v1', null)`, es decir, en la clave vieja sin aislar `staffgate_cands_v1`. El guardado en la nube (`syncToCloud()`, línea 995) no se ve afectado porque usa `session.user.id` fresco, así que no hay mezcla de cuentas por lectura — pero esa fila queda huérfana en la clave vieja, y como `isSyncKey()` (línea 11) la reconoce igual por el prefijo, también se replica al respaldo de carpeta local, donde sí queda expuesta a cualquier otra cuenta que conecte esa misma carpeta. *Decisión pendiente: ¿deshabilitar los controles de guardado hasta que `cachedUserId` esté confirmado, en vez de solo gatear la subida a la nube con `cloudSyncReady`?*

13. **(accessibility-reviewer, 2026-07-28, rama `claude/widget-cuenta-y-botones-mentor`) El nuevo logo del dardo (`AIAPPS_LOGO_DART`, `auth-gate.js`) puede salir del viewport en pantallas angostas, no solo del borde de la tarjeta.** Calculado por aritmética directa sobre el CSS real: el dardo viaja hasta `translate(9.6em,-2.8em)` desde su ancla (≈230px a la derecha, 74px arriba, con `1em=24px` porque `AIAPPS_LOGO_DART` no define `AIAPPS_APP_LOGO_SIZE`), y `.card`/`.brand` no tienen `overflow:hidden` en ningún punto de `auth-gate.js`. En un viewport de ~300-350px de ancho (`.card` usa `width:min(320px,90vw)`), ese recorrido puede sacar al dardo del borde de la ventana misma, no solo de la tarjeta, quedando parcialmente recortado por el viewport en el punto de partida de su ciclo. No verificado en vivo (sin JS eval en esta sesión) — es aritmética sobre valores reales del CSS, no una suposición. Detalle completo y consideración sobre la amplitud del recorrido en `docs/staffgate/accessibility-notes.md` §1. *Decisión pendiente: ¿reducir el recorrido inicial del dardo en viewports angostos (o en general), o aceptar el recorte como parte del efecto "entra desde fuera de cuadro"?*

14. **(accessibility-reviewer, 2026-07-28, mismo commit) El widget compartido de Cuenta salta ~190px hacia arriba al abrir su panel, desde que cambió de ancla `top` a `bottom` para apilarse sobre la insignia de carpeta.** Mismo hallazgo transversal que las otras 4 apps (código 100% compartido en `auth-gate.js`) — detalle completo, aritmética del modelo de caja, y recomendación de arreglo (una sola regla CSS) en `docs/staffgate/accessibility-notes.md` §2. *Decisión pendiente: aplicar el arreglo sugerido o aceptar el salto.*

13. **(qa-lead, 2026-07-26, commit `40c43f5` — aplica a las 5 apps + hub por ser `auth-gate.js` compartido) El "aplanado instantáneo" de la tarjeta al activar `prefers-reduced-motion` en vivo depende de `MediaQueryList.addEventListener`, no soportado en navegadores muy antiguos (ej. Safari &lt;14, que solo tiene el `addListener` obsoleto).** El código ya lo comprueba de forma defensiva (`typeof tiltQuery.addEventListener === "function"`) y en ese caso simplemente no registra el listener de `change` — no rompe nada, pero degrada silenciosamente: en esos navegadores, si alguien activa "reducir movimiento" con el login ya abierto y la tarjeta inclinada, esta se queda inclinada hasta el próximo `mousemove` (en vez de aplanarse sola). Es el mismo patrón defensivo que ya usa el resto del archivo (ej. el zoom automático de StaffGate), así que no es una regresión de este commit — solo una limitación heredada del navegador, no comunicada al usuario en ningún lugar.
