...
13. **(security-reviewer, 2026-09-03, rama `claude/update-notification-banner-85im24`) El banner nuevo de "hay una versión nueva" (`update-notice.js`, todas las apps) agrega un botón "Refresh" que llama a `location.reload()` sin ningún `beforeunload`/confirmación — si aparece mientras hay un formulario a medio llenar (o cualquier estado no guardado) en esta app, un clic descarta ese trabajo sin aviso. No es un bug nuevo del formulario en sí (ninguna app de las 6 tenía protección `beforeunload` antes de este cambio — confirmado por grep, 0 coincidencias), pero el banner es la primera vía de la interfaz que invita activamente a recargar en cualquier momento, aumentando la probabilidad de que ocurra por accidente frente a un F5 manual. *Decisión pendiente: aceptar el riesgo (el banner es descartable con la "x" y no reaparece por la misma versión), o hacer que "Refresh" revise algo equivalente a "hay cambios sin guardar" antes de recargar — necesitaría que cada app exponga esa señal, hoy no existe.* Detalle completo en `docs/security-notes.md` ("Revisión 2026-09-03 — `update-notice.js`").
