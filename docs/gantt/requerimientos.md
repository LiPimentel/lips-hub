...
12. **(security-reviewer, 2026-07-23) Nota: el punto 1 de esta lista (cronograma sin ningún guardado) ya fue resuelto por el commit `65ad3e7`, que agrega persistencia en Supabase + caché local + carpeta local, siguiendo el mismo patrón que las otras 4 apps.** Sin embargo, esa nueva persistencia hereda la misma ventana de carrera estrecha encontrada en StaffGate, LP-Bag y MyTravel: los botones "+ Agregar fila" (línea 589) y "Limpiar todo" (línea 595), y la edición de celdas (línea 573), son interactivos desde que la página pinta — no dependen de que `load()` termine. `cachedUserId` solo se fija dentro de `load()`, que se invoca de forma asíncrona en el IIFE de arranque (línea 801). Si el usuario edita una fila antes de que esa asignación ocurra (por ejemplo, si `getSupabaseSession()` tarda por una renovación de token), `save()` escribe con `scopedKey('gantt_v1', null)` — la clave vieja sin aislar. La nube no se ve afectada (usa `session.user.id` fresco en `syncToCloud()`), pero esa fila queda huérfana en la clave vieja y, al coincidir con el prefijo de `isSyncKey()` (línea 11), se replica también en el respaldo de carpeta local, quedando expuesta a cualquier otra cuenta que conecte esa carpeta. *Decisión pendiente: ¿deshabilitar los controles de guardado hasta que `cachedUserId` esté confirmado, en las 4 apps a la vez (patrón compartido), en vez de parchear una por una?*

13. **(qa-lead, 2026-07-24, revisando commit `7e6dac5` "barras escalonadas lila/amarillo/púrpura + logo animado") El logo animado del cuadro de login usa 3 colores (lila, amarillo Y púrpura), no los 2 que pidió la usuaria ("en lila y amarillo").** Verificado en vivo con `getComputedStyle`: la tercera barra del SVG (`.lb-3`) tiene `fill:#8C6BC8`, que es un tono púrpura/violeta (visualmente distinto de la barra lila `#B79BE0` y claramente no amarillo), calculado en `rgb(140, 107, 200)` sobre el elemento real. La petición original solo mencionó "lila y amarillo" para el logo (a diferencia de las barras del fondo, donde sí pidió tres colores explícitamente: lila, amarillo y púrpura). *Decisión pendiente: ¿la usuaria acepta el tercer color en el logo (para que combine con la paleta de tres colores del fondo) o quiere que el logo se quede estrictamente en dos colores como pidió?*

14. **(qa-lead, 2026-07-24, mismo commit) En viewport móvil (375×812), la escena decorativa del login (`.gantt-scene`, las barras escalonadas del fondo) queda 100% oculta detrás de la tarjeta de login.** Medido con `getBoundingClientRect()` real: a 375px de ancho, la escena ocupa x:23–165px y la tarjeta (fondo opaco `rgb(239,234,220)`, sin transparencia ni `backdrop-filter`, `z-index:4`) ocupa x:0–345px — la escena completa queda tapada. A 1280px de ancho no hay superposición (escena x:77–497px, tarjeta x:802–1178px). **Esto no es una regresión de este commit** — el posicionamiento de `.gantt-scene`/`.card`/`.cover.align-right` no cambió en el diff — pero es relevante porque toda la inversión de esta tarea (barras escalonadas de tres colores) es invisible para cualquier usuaria que abra el login desde el teléfono. Mismo patrón de layout (`AIAPPS_LOGIN_LAYOUT='right'`) que Bitácora del Mentor, así que probablemente afecta a esa app también. *Decisión pendiente: ¿ocultar la escena decorativa en móvil como ya se hace con `.growth-scene` (`@media max-width:900px{ display:none }`), o darle transparencia a la tarjeta en móvil para que se alcance a ver algo detrás?*
