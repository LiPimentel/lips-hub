...
- **2026-07-23/24 — mezcla de cuentas en computadora compartida (Bitácora, StaffGate, MyTravel, LP-Bag), y ventana de carrera introducida al corregirlo:** business-analyst encontró el patrón original (cuenta nueva sin fila en Supabase cae a caché `localStorage` sin verificar `user_id`) en las 4 apps que persisten datos. Corregido en `e421ee8` (caché/carpeta local aislada por `"<key>::userId"`) + `65ad3e7` (Gantt agregó persistencia completa con el aislamiento desde el inicio, así que nunca tuvo el bug). `security-reviewer` encontró que el fix dejaba una ventana de carrera (guardado disparado antes de que `cachedUserId` se confirmara caía en la clave vieja) — corregido en `a4a2d10` (cada `save()` resuelve la sesión de nuevo en vez de confiar en la variable cacheada). Detalle completo en `docs/security-notes.md`.
