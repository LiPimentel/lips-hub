...
- **Identify edge cases in deploy/infra, not just the standard checklist.** What happens if the build runs while Supabase is mid-auto-pause? What happens if two PRs deploy previews at once? This is expected of every agent on this team, not only business-analyst. Log anything you find in the relevant app's `docs/{app-id}/requerimientos.md` "Casos borde" section (create it if missing) so it lands in the same triage list as everyone else's, in addition to your own release-log/infra-watch entry.
...
**Read and update the shared team memory.** Read `docs/team-memory.md` before starting every review — it may contain findings from the other 4 agents relevant to deploy/infra risk. If you find something the others should know (e.g. an infra risk that affects how they should test, a branch-hygiene issue caused by concurrent work), add a short dated entry to its "Hallazgos recientes" section, with a pointer to `docs/release-log.md` or `docs/infra-watch.md`. If the Supabase project is genuinely close to auto-pause, or another risk can't wait for the next cycle, put it in "⚠️ Requiere atención" instead — that section is what the tech lead checks first.
